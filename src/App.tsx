...
import { Plus, Trash2, Calendar, Clock, AlignLeft, GripVertical, Percent, Palette } from 'lucide-react';
...
const initialTasks: Task[] = [
  { id: '1', name: 'Stud Wall Construction (80h)', startDate: '2026-04-07', duration: 10, color: 'bg-blue-500', progress: 100 },
  { id: '2', name: 'First Fix Plumbing', startDate: '2026-04-17', duration: 1, color: 'bg-cyan-500', progress: 100 },
  { id: '3', name: 'Ceiling Installation (32h)', startDate: '2026-04-18', duration: 4, color: 'bg-purple-500', progress: 50 },
  { id: '4', name: 'Second Fix Plumbing / Radiators', startDate: '2026-04-22', duration: 1, color: 'bg-cyan-500', progress: 0 },
  { id: '5', name: 'Decoration (16h)', startDate: '2026-04-23', duration: 2, color: 'bg-pink-500', progress: 0 },
];

const colorPalette = [
  'bg-blue-500', 'bg-cyan-500', 'bg-teal-500', 'bg-green-500', 
  'bg-yellow-500', 'bg-orange-500', 'bg-red-500', 'bg-pink-500', 'bg-purple-500', 'bg-indigo-500', 'bg-slate-500'
];
...
export default function App() {
  ...
  // --- State Handlers ---
  const handleTaskChange = (id: string, field: keyof Task, value: string | number): void => {
    setTasks(tasks.map(task => {
      if (task.id !== id) return task;
      
      let parsedValue = value;
      if (field === 'duration') parsedValue = Math.max(1, parseInt(value) || 1);
      if (field === 'progress') parsedValue = Math.min(100, Math.max(0, parseInt(value) || 0));
  ...
  const addTask = () => {
    ...
    const newStartDate = lastTask ? addDays(lastTask.startDate, lastTask.duration) : new Date().toISOString().split('T')[0];
  ...
  const { projectStart, projectEnd, totalDays, timelineDates } = useMemo(() => {
    if (tasks.length === 0) {
      ...
      return { projectStart: today, projectEnd: today, totalDays: 1, timelineDates: [new Date(today)] };
    }

    const startDates = tasks.map(t => new Date(t.startDate));
    const endDates = tasks.map(t => new Date(addDays(t.startDate, t.duration)));
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Project Schedule: SNF Water Science Offices</h1>
            <p className="text-slate-500 mt-1">Quote Ref: #QUO09407 | Admin Console & Gantt Chart</p>
          </div>
          <button 
            onClick={addTask}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full max-h-[700px]">
              <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold text-slate-800">Admin Console</h2>
                </div>
                  ...
                  tasks.map((task) => (
                    <div 
                      ...
                      className={`group relative bg-white border rounded-xl p-3 shadow-sm transition-all
                        ${draggedId === task.id ? 'border-blue-400 shadow-md ring-2 ring-blue-100' : 'border-slate-200 hover:border-blue-300'}
                      `}
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        ...
                        <div className="flex gap-2">
                          <div className="flex flex-1 items-center gap-1.5 text-slate-500 bg-slate-50 p-1.5 rounded-md border border-slate-100 focus-within:border-blue-300 focus-within:bg-white transition-colors">
                            ...
                            />
                            <span className="text-[10px] uppercase font-semibold text-slate-400 pr-1">Days</span>
                          </div>
                          <div className="flex flex-1 items-center gap-1 text-slate-500 bg-slate-50 p-1.5 rounded-md border border-slate-100 focus-within:border-blue-300 focus-within:bg-white transition-colors">
                            ...
                            <input
                              ...
                              max="100"
                              value={task.progress}
                              onChange={(e) => handleTaskChange(task.id, 'progress', e.target.value)}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
              <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold text-slate-800">Timeline view</h2>
                </div>
              </div>
                ...
                <div className="min-w-max">
                  ...
                  >
                    ...
                    {timelineDates.map((date, index) => {
                      ...
                      return (
                        <div 
                          key={index} 
                          className={`bg-slate-50 p-2 text-center flex flex-col items-center justify-end h-14 ${isWeekend ? 'bg-slate-100' : ''}`}
                        >
                          <span className="text-[10px] font-semibold text-slate-400 uppercase">
                            {date.toLocaleDateString('en-GB', { weekday: 'short' })}
                          </span>
                          <span className={`text-xs font-medium ${isWeekend ? 'text-slate-400' : 'text-slate-700'}`}>
                            {date.getDate()}
                          </span>
                        </div>
                      );
                  ...
                  <div className="border border-slate-200 border-t-0 rounded-b-lg overflow-hidden bg-slate-50 relative">
                    ...
                    >
                      ...
                      {timelineDates.map((date, index) => (
                        <div key={index} className={`border-r border-slate-200 ${date.getDay() === 0 || date.getDay() === 6 ? 'bg-slate-100/50' : ''}`}></div>
                      ))}
                    </div>
                    ...
                    <div className="relative z-10">
                      {tasks.map((task, rowIndex) => {
                        ...
                        const projectStartStr = projectStart instanceof Date ? projectStart.toISOString().split('T')[0] : projectStart;
                        const startColIndex = getDaysDifference(projectStartStr, task.startDate) + 2; 
                        const endDate = addDays(task.startDate, task.duration - 1);
                        
                        return (
                          <div 
                            key={task.id}
                            className={`grid gap-px border-b border-slate-100 relative group transition-colors hover:bg-slate-100/50 ${rowIndex % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}
                            style={{ gridTemplateColumns: `240px repeat(${totalDays}, minmax(40px, 1fr))` }}
                          >
                            ...
                            >
                               <div className={`w-2.5 h-2.5 rounded-full mr-2 flex-shrink-0 ${task.color}`}></div>
                               <span className="truncate">{task.name || 'Unnamed Task'}</span>
                            </div>
              ...
              <div className="p-3 bg-slate-50 border-t border-slate-100 text-xs text-slate-500 flex flex-wrap gap-4 items-center">
                <span className="flex items-center gap-1.5"><div className="w-3 h-3 bg-slate-100 border border-slate-200 rounded-sm"></div> Weekends</span>
                ...
                <span className="ml-auto flex items-center gap-1 text-slate-400">
                  <Palette size={14}/> Click task colors in admin panel to change
                </span>
              </div>
            </div>
          </div>
